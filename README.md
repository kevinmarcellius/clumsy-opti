...
While a widget is installed, a one-off deferrable `JobScheduler` job is requested about two minutes after each attempt. It requires an available Android network, checks that the screen is interactive, and reads account data over the network assigned to that job. Android may delay, throttle, or stop the job; no foreground service is used. The widget always shows the last successful fetch time and marks data stale after ten minutes when it is redrawn. Widget metadata updates are disabled because Android does not deliver `updatePeriodMillis` more often than about 30 minutes.
